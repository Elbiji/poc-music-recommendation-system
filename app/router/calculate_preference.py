...
async def calculate_preference(
    ...
    # Get track histories
    user_track_history_cursor = (
        collection.find(query_filter).sort("createdAt", -1).limit(20)
    )
    ...
    update_action = {"$set": {"profile_vector": user_preference_profile}}

    # Execute the update
    result = await db.users.update_one(query_filter, update_action, upsert=False)
