...
async def save_to_db(data, user):
    ...
    songs_df["played_at"] = pd.to_datetime(songs_df["played_at"]).dt.strftime(
        "%d/%m/%y"
    )
...
async def recently_played(
    ...
    r = requests.get(
        settings.API_BASE_URL + "me/player/recently-played", headers=headers
    )

    recently_played_tracks = r.json()

    await save_to_db(recently_played_tracks, user_id)
    return JSONResponse(content=recently_played_tracks, status_code=status.HTTP_200_OK)
