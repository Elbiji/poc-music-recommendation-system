...
def getUser(access_token: str) -> JSONResponse:
    ...
    try:
        response = requests.get(settings.API_BASE_URL + "me", headers=headers)
        ...
        if response.status_code == 200:
            ...
            return JSONResponse(
                ...
                content={
                    "message": "Invalid access token for accessing Spotify API service."
                },
            )
        else:
            return JSONResponse(
        ...
        if response.status_code == 200:
            ...
            update_action = {
                "$set": {
                    ...
                    + timedelta(seconds=token_info.get("expires_in")),
                }
            }
...
async def callback(code: str | None = None, error: str | None = None) -> JSONResponse:
    ...
    if code:
        ...
        if response.status_code != 200:
            return JSONResponse(
                {"error": "Token exchange failed"}, status_code=response.status_code
            )
        ...
        if not result.acknowledged:
            return JSONResponse(
                status_code=500, content={"message": "Mongodb internal server error"}
            )
